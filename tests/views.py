...
from .models import Mp_location_test
...
def manpower_test(request):
  return render(request, 'test/manpower_test_home.html')

@login_required()
def mp_have_move_req(request):
  return render(request, 'test/mp_have_move_req.html')
...
def mp_move_list(request):
  ...
  return render(request, 'test/mp_move_list.html', context)
...
def manpower_test_move(request):
  if request.method == "POST":
    ...
    form = Mp_move()
  context = {"form" : form}
  return render(request, 'test/manpower_test_move.html', context)
